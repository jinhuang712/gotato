...
func main() {
	...
	maxAgents := flag.Int("max-agents", 0, "maximum live Agents; 0 disables the bound")
	...
	defaultTools := []gotato.Tool{}
	if *modelName == "demo" {
		defaultTools = append(defaultTools, echoTool{})
	}
	definitions := []orchestration.Definition{
	...
	if *queuePolicy == string(orchestration.QueueFIFO) {
		queue = orchestration.QueueFIFO
